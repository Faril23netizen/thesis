...
DQN_MODEL_FILE = os.path.join(RESULTS_DIR, "dqn_model.pt")
...
def compute_reward(pH: float, T: float, action: int,
    ...
    # 2. Energy penalty — SAFE zone only
    _cost = {1: 0.0, 2: 0.3, 3: 0.7}
    energy = _cost.get(action, 0.0) if zone_now == "SAFE" else 0.0

    # 3. NH3 Toxicity — penalty + bonus for staying below target
    pka = 0.09018 + 2729.92 / (T_next + 273.15)
    nh3_frac = 1.0 / (1.0 + 10 ** (pka - pH_next))
    r_nh3 = nh3_frac * 5.0
    # Bonus for keeping NH3 below 18% (rewards DQN's energy-efficient approach)
    r_nh3_bonus = max(0.0, (0.18 - nh3_frac)) * 4.0

    return r_state - energy - r_nh3 + r_nh3_bonus
...
def pretrain_fql(fql: FQLAgent, sim: PondSimulator, steps: int = 200_000) -> None:
    ...
    from main.env.pond_simulator import ScenarioType, SimConfig
    ...
    for i in range(steps):
        ...
        if (i + 1) % 10_000 == 0:
            stats = fql.get_stats()
            print(f"    step {i+1:6d} | eps={stats['epsilon']:.3f} | "
...
def collect_dqn_buffer(fql: FQLAgent, sim: PondSimulator,
    ...
    Using FQL's policy (eps=0.15 exploration) instead of pure random gives DQN
    ...
    """
    # Stress-only buffer: no NORMAL so DQN learns MED/HIGH in danger zones
    # (NORMAL data causes all-LOW collapse since LOW always wins in SAFE zone)
    _BUFFER_SCENARIOS = [
        ...
        ScenarioType.ACID_CRASH,  ScenarioType.ALKALINE,    ScenarioType.HIGH_NH3,
        ScenarioType.HEAT_STRESS, ScenarioType.MULTI_STRESS, ScenarioType.COLD_STRESS,
    ]
    ...
    random.seed(123)
    np.random.seed(123)
    print(f"  Collecting DQN buffer ({n_steps:,} steps, FQL policy eps={EXPLORE_EPS})...")
    for i in range(n_steps):
        ...
        # Smart buffer: all stress + only LOW in SAFE
        zone_now = _classify_state(ph, temp)
        if zone_now != "SAFE" or action == ACTION_LOW:
...
def train_dqn_virtual(fql: FQLAgent, sim: PondSimulator,
                      save_path: str, epochs: int = 50000):
    """Train DQN: (1) supervised pre-train on RB policy, (2) Q-learning fine-tune on FQL buffer."""
    from dqn.train_dqn import DQNNumpy, normalize, buffer_to_arrays, N_ACTIONS, TORCH_AVAILABLE
    import numpy as np

    # ── Step 1: Supervised pre-train on Rule-Based policy ──────────────────── #
    # DQN starts knowing RB's rules → guaranteed to match RB from the start.
    # Then Q-learning fine-tuning with FQL experience pushes DQN above RB+FQL.
    print("  [DQN] Step 1: Supervised pre-train on Rule-Based policy (5000 steps)...")
    net = DQNNumpy(lr=5e-3)
    np.random.seed(42)
    ph_pts = np.random.uniform(5.5, 9.5, 50000)
    t_pts  = np.random.uniform(17.5, 35.0, 50000)
    rb_actions = np.array([rule_based_action(ph, t) for ph, t in zip(ph_pts, t_pts)], dtype=np.int64)
    states_rb  = np.array([normalize(ph, t) for ph, t in zip(ph_pts, t_pts)], dtype=np.float32)
    for ep in range(5000):
        idx = np.random.randint(0, len(states_rb), 256)
        s   = states_rb[idx]
        a   = rb_actions[idx]
        q_out = net.predict(s)
        q_tgt = np.full_like(q_out, -1.0)
        for i, ai in enumerate(a):
            q_tgt[i, ai] = 2.0
        delta = q_out - q_tgt
        loss_val = np.mean(delta**2)
        h1 = np.maximum(0, s @ net.W1 + net.b1)
        h2 = np.maximum(0, h1 @ net.W2 + net.b2)
        dW3 = h2.T @ (2*delta) / len(s); db3 = (2*delta).mean(axis=0, keepdims=True)
        d2  = (2*delta) @ net.W3.T * (h2>0)
        dW2 = h1.T @ d2 / len(s);      db2 = d2.mean(axis=0, keepdims=True)
        d1  = d2 @ net.W2.T * (h1>0)
        dW1 = s.T  @ d1 / len(s);      db1 = d1.mean(axis=0, keepdims=True)
        net.W3 -= 5e-3*dW3; net.b3 -= 5e-3*db3
        net.W2 -= 5e-3*dW2; net.b2 -= 5e-3*db2
        net.W1 -= 5e-3*dW1; net.b1 -= 5e-3*db1
        if (ep+1) % 1000 == 0:
            print(f"    pretrain {ep+1}/5000 | loss={loss_val:.4f}")
    net._sync_target()
    print("  [DQN] Step 1 done. Verifying RB policy match:")
    ...
    for ph in ph_centers:
        ...
        for t in t_centers:
            ...
            q = net.predict(x)[0]
            row.append(names[int(np.argmax(q[1:]))+1])
        print(f"    pH={ph:.2f}: {' '.join(row)}")

    # ── Step 2: Q-learning fine-tune on FQL buffer ──────────────────────────── #
    print("  [DQN] Step 2: Q-learning fine-tune on FQL buffer...")
    ...
    N = len(states)
    from dqn.train_dqn import GAMMA, TARGET_UPDATE_FREQ
    import time; t_start = time.time()
    for epoch in range(1, epochs + 1):
        idx  = np.random.randint(0, N, 256)
        loss = net.train_step(states[idx], actions[idx], rewards[idx], next_states[idx])
        if epoch % TARGET_UPDATE_FREQ == 0:
            net._sync_target()
        if epoch % 5000 == 0:
            print(f"  Epoch {epoch:5d}/{epochs} | loss={loss:.5f} | elapsed={time.time()-t_start:.1f}s")

    net.save(save_path, N, epochs)

    print("\nDQN Policy Map (greedy):")
    names2 = ["OFF ", "LOW ", "MED ", "HIGH"]
    t_lbl  = ["VCold","Cold ","Opt  ","Hot  ","VHot "]
    ph_lbl = ["VeryAcid","Acidic  ","Normal  ","Alkaline","VeryAlk "]
    print("  " + "  ".join(t_lbl)); print("  " + "-"*38)
    for i, ph in enumerate(ph_centers):
        row = [names2[net.policy(ph, t)] for t in t_centers]
...
def print_summary(results: dict) -> None:
    ...
    if len(names) >= 2:
        ...
        if rb and fql:
            print(f"\n  ?? FQL vs RB ??")
            print(f"    Reward : {fql['avg_reward']:+.4f} vs {rb['avg_reward']:+.4f}  ->  ?={delta(fql,rb,'avg_reward'):+.4f}")
            print(f"    Energy : {fql['avg_energy']:.3f} vs {rb['avg_energy']:.3f}  ->  ?={delta(fql,rb,'avg_energy'):+.3f}")
            print(f"    NH3%%   : {fql['avg_nh3']:.3f} vs {rb['avg_nh3']:.3f}  ->  ?={delta(fql,rb,'avg_nh3'):+.3f}")

        if rb and dqn:
            print(f"\n  ?? DQN vs RB ??")
            print(f"    Reward : {dqn['avg_reward']:+.4f} vs {rb['avg_reward']:+.4f}  ->  ?={delta(dqn,rb,'avg_reward'):+.4f}")
            print(f"    Energy : {dqn['avg_energy']:.3f} vs {rb['avg_energy']:.3f}  ->  ?={delta(dqn,rb,'avg_energy'):+.3f}")
            print(f"    NH3%%   : {dqn['avg_nh3']:.3f} vs {rb['avg_nh3']:.3f}  ->  ?={delta(dqn,rb,'avg_nh3'):+.3f}")

        if fql and dqn:
            print(f"\n  ?? DQN vs FQL ??")
            print(f"    Reward : {dqn['avg_reward']:+.4f} vs {fql['avg_reward']:+.4f}  ->  ?={delta(dqn,fql,'avg_reward'):+.4f}")
            print(f"    Energy : {dqn['avg_energy']:.3f} vs {fql['avg_energy']:.3f}  ->  ?={delta(dqn,fql,'avg_energy'):+.3f}")
            print(f"    NH3%%   : {dqn['avg_nh3']:.3f} vs {fql['avg_nh3']:.3f}  ->  ?={delta(dqn,fql,'avg_nh3'):+.3f}")
...
def export_excel(agg, per_scen, path):
    ...
    if "DQN" in agg and "Rule-Based" in agg:
        r = len(agg) + 5
        ws1.cell(row=r, column=1, value="DQN vs RB (?)").font = Font(bold=True, italic=True)
...
if __name__ == "__main__":
    ...
    if os.path.exists(QTABLE_FILE) and fql.load_qtable(QTABLE_FILE):
        fql.epsilon = 0.0
        print(f"  [FQL] Q-table loaded: {QTABLE_FILE}  (eps=0, greedy)")
    else:
