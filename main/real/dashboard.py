...
<body>
    <div class="container">
        ...
        <!-- Tabs (Dynamically generated by JS) -->
        <div class="tabs" id="pico-tabs">
    ...
    <script>
        ...
        let currentKnownNodes = [];
        function updateTabs(nodeKeys) {
            // Check if node list changed
            const nodeNames = nodeKeys.map(k => {
                if (k.includes('1')) return 'Pico 1';
                if (k.includes('2')) return 'Pico 2';
                if (k.includes('3')) return 'Pico 3';
                return k;
            }).filter((v, i, a) => a.indexOf(v) === i); // unique
            
            // Only update DOM if the nodes changed
            if (JSON.stringify(nodeNames) !== JSON.stringify(currentKnownNodes)) {
                currentKnownNodes = nodeNames;
                const tabsContainer = document.getElementById('pico-tabs');
                tabsContainer.innerHTML = '';
                
                nodeNames.forEach(name => {
                    const btn = document.createElement('button');
                    btn.className = 'tab-btn' + (name === activePico ? ' active' : '');
                    btn.innerText = name;
                    btn.onclick = () => selectPico(name);
                    tabsContainer.appendChild(btn);
                });

                // Ensure activePico is still valid
                if (!nodeNames.includes(activePico) && nodeNames.length > 0) {
                    selectPico(nodeNames[0]);
                }
            }
        }
            ...
            try {
                ...
                if (netRes.ok) {
                    ...
                    if (!net.error || net.error === "Network stats are stale") {
                        ...
                        if (net.nodes && Object.keys(net.nodes).length > 0) {
                            // Dynamically update tabs if the number of nodes changes
                            updateTabs(Object.keys(net.nodes));
